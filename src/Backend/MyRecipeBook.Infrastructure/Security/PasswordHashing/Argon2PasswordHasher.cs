...
internal sealed class Argon2PasswordHasher : IPasswordHasher
{
    ...
    public string HashPassword(string password)
    {
        ...
        var combineBytes = new byte[hash.Length + salt.Length];

        salt.CopyTo(combineBytes);
        hash.CopyTo(combineBytes, index: salt.Length);
    ...
    {
        var combineBytes = Convert.FromBase64String(passwordHash);

        var salt = new byte[SALT_SIZE];
        var hash = new byte[HASH_SIZE];

        Array.Copy(combineBytes, salt, SALT_SIZE);
        Array.Copy(combineBytes, SALT_SIZE, hash, 0, HASH_SIZE);
